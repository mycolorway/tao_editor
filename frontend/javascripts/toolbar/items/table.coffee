...
class TableItem extends MenuItem
  ...
  _bind: ->
    ...
    @on 'click', '.table-creator .link-create-table', (e) =>
      $td = $(e.currentTarget).closest('td')
      [rowIndex, colIndex] = @_getCellPosition $td
      Commands.insertTable @editorView.state, @editorView.dispatch, rowIndex + 1, colIndex + 1
      @editorView.focus()
      @_resetTableCreator()
      @_hidePopover()
      null
  ...
  _updateDisabled: ->
    @disabled = !@active && !Commands.insertTable(@editorView.state, null)

export default TableItem.register()
